...
```bash
# 构建镜像（默认镜像名为 verdaccio:6.5.0）
docker build -t verdaccio:6.5.0 .
```
...
```bash
# 导出镜像为 tar 文件
docker save -o verdaccio.tar verdaccio:6.5.0
```
...
- **原因**：宿主机 `data` 目录权限不正确。
- **解决**：再次运行 `sudo chown -R 10001:65533 ./data`。
